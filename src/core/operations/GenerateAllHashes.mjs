...
class GenerateAllHashes extends Operation {
    ...
    formatDigest(digest, length, includeNames, name) {
        // FORK CHANGE (patches/fork/07): an unavailable-algorithm report is not a digest, so the
        // length filter must not swallow it. `[unavailable: ...]` has no bit length, so
        // `digest.length * 4` never matches a requested size and the line vanished for every
        // length except "All" -- which silently reinstated the failure this patch exists to fix.
        const unavailable = digest.startsWith("[unavailable:");
        if (!unavailable && length !== "All" && (digest.length * 4) !== parseInt(length, 10))
            return "";
