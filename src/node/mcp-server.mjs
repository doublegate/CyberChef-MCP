...
function sanitizeToolName(name) {
  if (!name) return null;
  let sanitized = "cyberchef_" + name.toLowerCase()
    .replace(/[^a-z0-9_]/g, "_")
    .replace(/_+/g, "_")
    .replace(/^_|_$/g, "");
...
// Resolve the value of an argument: handling defaults and name-to-value mapping
function resolveArgValue(argDef, userValue) {
    // 1. Handle Defaults if userValue is undefined
    if (userValue === undefined) {
        let defaultVal = argDef.value; // Fallback
    ...
    if ((argDef.type === 'option' || argDef.type === 'editableOption') && Array.isArray(argDef.value)) {
        ...
        if (argDef.type === 'option') {
            ...
            // Unless zod validation was skipped or mismatch.
            // But if Zod said it's OK, then it must be one of the names.
            // So we return userValue (which is the name). 
            // WAIT. If the option value IS the name (e.g. array of strings), then returning userValue is correct.
            // If option is object {name, value}, then userValue is NAME. We returned match.value above.
            return userValue; 
        }
