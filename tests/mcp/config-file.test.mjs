...
describe("resolving a configuration", () => {
    ...
    it("covers every setting the server reads", () => {
        ...
        // 61 since v4.0.0, down from 64: the `compatibility` section (V2_COMPATIBILITY_MODE and
        // CYBERCHEF_SUPPRESS_DEPRECATIONS) went with the deprecation system it configured, and
        // `tools.exposeAllOps` went with the inert CYBERCHEF_EXPOSE_ALL_OPS alias -- leaving the
        // mapping would have let a config file set a variable that nothing reads, which is worse
        // than removing it, because the file fails closed and says so. Reviewer-found.
