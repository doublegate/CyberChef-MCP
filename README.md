...
**Latest Release:** v3.10.0 | [Release Notes](docs/releases/v3.10.0.md) | [Tutorial](docs/guides/tutorial.md) | [Examples](examples/) | [Breaking Changes](docs/v2.0.0-breaking-changes.md) | [Security Policy](SECURITY.md)
...
*   **All 504 operations, without paying for 504 schemas** (v2.1.0): `tools/list` is an **index** by default — 42 tools and 44,493 bytes, rather than 545 tools and 424,897 bytes. The index grew in v3.3.0 because twelve new registry tools have no navigation path of their own — a registry tool that is not listed cannot be called at all. Every operation stays reachable: `cyberchef_categories` -> `cyberchef_list_operations` -> `cyberchef_describe_operation` walks down to any of them, `cyberchef_search` finds one by keyword, and `cyberchef_bake` runs any of them by name. `CYBERCHEF_TOOL_SURFACE=curated` (120 tools, 107,739 bytes) or `=all` (all 545, 424,897 bytes) if you would rather pre-load. See the [User Guide](docs/guides/user_guide.md#the-tool-surface--how-many-tools-you-see-and-why).
...
1.  **Download the tarball** (**141 MB** compressed; measured against the published v3.1.0 asset, not estimated):
    ...
    wget https://github.com/doublegate/CyberChef-MCP/releases/download/v3.10.0/cyberchef-mcp-v3.10.0-docker-image.tar.gz
    ```

2.  **Load the image into Docker:**

    ```bash
    docker load < cyberchef-mcp-v3.10.0-docker-image.tar.gz
    ```
