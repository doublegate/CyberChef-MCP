...
**Latest Release:** v3.8.0 | [Release Notes](docs/releases/v3.8.0.md) | [Tutorial](docs/guides/tutorial.md) | [Examples](examples/) | [Breaking Changes](docs/v2.0.0-breaking-changes.md) | [Security Policy](SECURITY.md)
...
*   **All 504 operations, without paying for 504 schemas** (v2.1.0): `tools/list` is an **index** by default — 41 tools and 44,406 bytes, rather than 545 tools and 424,810 bytes. The index grew in v3.3.0 because twelve new registry tools have no navigation path of their own — a registry tool that is not listed cannot be called at all. Every operation stays reachable: `cyberchef_categories` -> `cyberchef_list_operations` -> `cyberchef_describe_operation` walks down to any of them, `cyberchef_search` finds one by keyword, and `cyberchef_bake` runs any of them by name. `CYBERCHEF_TOOL_SURFACE=curated` (119 tools, 107,652 bytes) or `=all` (all 544, 424,810 bytes) if you would rather pre-load. See the [User Guide](docs/guides/user_guide.md#the-tool-surface--how-many-tools-you-see-and-why).
