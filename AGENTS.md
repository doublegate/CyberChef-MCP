...
**CyberChef MCP Server** (v4.2.0) - Fork of GCHQ CyberChef wrapping the Node.js API into an MCP
server. Exposes 504 operations (encryption, encoding, compression, forensics) as AI assistant tools.

| Metric | Value |
|--------|-------|
| MCP Version | 4.2.0 (single source: `package.json` `version`, read by `src/node/lib/config.mjs`). `mcpVersion` was removed in v2.2.0 -- npm requires `version` to be the published version, so the upstream base moved to `cyberchefUpstreamVersion`. |
| Upstream base | CyberChef **v11.4.0** |
| Operations / tools | 504 operations **plus 19 registry tools** that are not operations. `tools/list` is an **index** by default (23 tools, **15,620 bytes**); `CYBERCHEF_TOOL_SURFACE=curated\|all` for 120 (109,548) or all 545 (426,706). Every figure here was 325 bytes low through v3.3.0 -- recorded during its development and never re-measured before the tag, which is this row's own warning happening to this row. The index **doubled in v3.3.0** and fell by 66% in **v4.1.0**, and both moves are the same fact: a registry tool had no navigation path, because `describe_operation` refused it and pointed at `tools/list`, so the listing was its only schema path and an unlisted tool could not be called at all. Nineteen of them were **30,683 of 44,968 bytes, 68%** of the surface whose purpose is being small. v4.1.0 removed the dependency rather than the tools -- `describe_operation` serves their schemas, `categories` lists them, `cyberchef_analyse` dispatches to them -- so the growth curve is now **flat**: a twentieth or a hundredth leaves the index unchanged, and `index-growth.test.mjs` gates that in both directions. The charter's claim that the index would cost more than `curated` at 59 registry tools was **arithmetically impossible** and is recorded as F-02: registry tools are listed on every surface, so they grew all three equally and the gap never moved. The tool COUNT is the half that was missing -- Anthropic measures tool-selection accuracy degrading past **30-50 tools**, and at 41 this index was inside that band. Listing must never be stricter than dispatch. Bytes, not tokens: no tokenizer has ever been in this repo and every historical `~N tokens` figure was bytes/4. Re-measure with `npm run measure:surfaces` rather than trusting this row -- every number in `tool-catalog.mjs`'s header had drifted by v3.1.0. All 504 reachable via `cyberchef_bake` + the three navigation tools. Every tool carries annotations + a title. |
| Licence | **GPL-3.0-or-later** (from v2.0.0; v1.9.x and earlier are Apache-2.0) |
| Node | **`>=26 <27` since v4.0.0**; image runs Node 26.8.2, digest-pinned (re-resolved in v3.9.0; `latest-slim` is still v25.9.0, so the shell-free base stays declined a third release). The floor was `>=24` through v3.11.0, and CI tested BOTH ends of that range since v2.8.1 -- before that every workflow tested 24 while the image shipped 26, so the runtime users get was never exercised. With floor and ceiling now one major apart there is one runtime to test, which is also the one that ships. |
...
| Magic | `cyberchef_magic` does NOT go through the operation; `src/node/lib/magic.mjs` calls `speculativeExecution` directly and returns a plain-text report + `structuredContent`. Reason: the operation emitted the web results **table**, with its recipes in the pretty form (`From_Base64('A-Za-z0-9+/=',true,false)`) which **`bake` rejects** -- the most actionable field was the one field a caller could not use. Recipes are now emitted as `[{op,args}]` and a test bakes every one. Language is an **estimate**, never a determination: chi-squared byte frequencies called "Attack at dawn" German at probability 1.35e-8, and accuracy does not rise monotonically with length, so no cutoff fixes it. This path bypasses `resolveArgValue`, so it screens the crib regex itself. **Candidates are re-ranked in v3.9.0** (issue #130): upstream's comparator clamps any file-signature match to 500 while text keeps chi-squared scores in the thousands, so a detected CONTAINER outranked the plaintext extracted from it -- a three-layer payload put the decoded answer at rank 3, beneath raw gzip. `promoteDecodings` in `magic.mjs` lifts a UTF-8 result above a container **whose recipe it extends**, and nothing else; the container anchor is load-bearing, because the first attempt promoted any longer UTF-8 result and handed rank 1 to a `From Hexdump` branch returning one character. Recipe length is NOT the cause and the arithmetic is in the v3.9.0 findings log. Fixed here, not in `src/core/lib/Magic.mjs`, which is mirrored verbatim. |
| Configuration | Two sources: `cyberchef.config.json` and environment variables, with **env > file > default**. 61 settings in 14 sections (the `compatibility` section and `tools.exposeAllOps` went with the deprecation system and the inert alias in v4.0.0); the mapping is a committed literal in `src/node/lib/config-file.mjs`, NOT a runtime transform (`http.maxSessions` is `CYBERCHEF_MAX_SESSIONS`, not `CYBERCHEF_HTTP_MAX_SESSIONS`, so a derived name would be set and read by nothing). Applied by populating `process.env` before anything reads it, because settings are resolved at MODULE LOAD in ~30 places across five modules -- hence `bootstrap-config.mjs` and the rule that it is imported FIRST. Fails closed on unknown sections/keys/types. Added in v2.10.0; the migration guide had told users to write this file since v1.8.0 and nothing read it. |
...
| Coverage | Measured in v4.2.0: **96.44% lines / 89.88% branches / 96.84% functions / 95.50% statements** -- every figure up on v4.1.0, and the two thinnest margins widened: statements clear the threshold by 0.50 (was 0.15) and branches by 0.88 (was 0.31). `mcp-server.mjs` still carries the deficit but less of it, at **85.56% statements / 70.39% branches** against 85.40%/68.04% before the dispatch consolidation -- which moved those by removing **23 statements and 8 branches** rather than by covering more, the outcome the sprint asked for. Two of its uncovered branches are deliberate: the table dispatch refuses a handler return that `JSON.stringify` cannot serialise, and no handler does that, so the guard is unreachable by construction. It is kept anyway because the alternative (`value ?? {}`) is covered and wrong -- it serves `{}` and turns a handler bug into a silent empty answer. Reviewer-suggested, verified to fire by making a handler return `undefined`. Previously v4.1.0 measured **96.14 / 89.31 / 96.57 / 95.15**. Previously v4.0.0 measured **96.08 / 89.43 / 96.54 / 95.1** -- essentially unmoved by deleting 522 lines of source and 1,188 of tests together, which is what deleting a well-covered subsystem looks like. Previously v3.11.0 measured **96.11 / 89.44 / 96.64 / 95.15**. **The thin margin v3.10.0 warned about was hit on the first release after it**: adding one registry tool took statements to 94.99% against a threshold of 95, with nothing deleted -- a file whose parser-rejection paths and explicit-format branches no test reached. It was fixed the way that row said to fix it, by covering the code rather than moving the bar, and the tests written to do it found a real defect (the tool raised a bare `Error` where every sibling raises `createInputError`, so the same class of bad input reached a client untyped). Statements now clear by 0.15 rather than 0.06, and `pqc-identify.mjs` itself sits at 99.18% statements / 100% lines. Previously v3.10.0 measured **96.02 / 89.13 / 96.6 / 95.06** -- **down ~0.6% from v3.9.0 with nothing deleted**, because `runServer()` used to execute on every test import (that was the defect) so its body counted as covered in every run. Roughly 0.6% of this project's line coverage was the bug executing itself, and the thresholds were calibrated partly against that. When a future change trips it, cover `mcp-server.mjs` (82.7% statements, the file `vitest.config.mjs` already names as carrying the deficit) rather than moving the threshold, which is now calibrated against a figure that is true. Previously v3.9.0 measured **96.56 / 89.61 / 96.6 / 95.53** -- unchanged to within a rounding step from v3.8.0 (96.6/89.5/96.5/95.5), which is the measurement that settled whether vitest 5's `coverage.include`/`exclude` relative-path change moved the gate. It did not -- quoted in the order vitest's own summary prints, because this row had lines and statements **transposed** (95.7 lines / 96.6 statements) for six releases, which is the exact confusion `vitest.config.mjs` carries a paragraph warning about. Thresholds raised in v2.3.0 from 75/70/90/75 to **96/89/96/95** (branches raised again in v2.7.0) -- that tuple is written lines/branches/functions/statements and matches `vitest.config.mjs`, which sets `lines: 96`, `functions: 96`, `branches: 89`, `statements: 95`. It read **95/89/96/96** until v3.10.0, i.e. lines and statements TRANSPOSED -- in the row that exists to document a past transposition of lines and statements. Reviewer-found, matching the config's named keys and NOT the order above. With `src/node/lib/**` held separately at 99/94/100/99. The old numbers were twenty points below actual, so the gate could not fail. `perFile` is off deliberately; see `vitest.config.mjs` for why. Gated on **pull requests** since v2.2.0. |
...
| Registry tools | **19** in `src/node/tools/`, for analyses an operation cannot express. **Not listed on the `index` surface since v4.1.0** -- reached via `cyberchef_analyse`, described by `cyberchef_describe_operation`, listed by `cyberchef_categories` under `analysisTools`, and still listed outright on `curated`/`all`. `index-growth.test.mjs` gates both directions, so the gate cannot be satisfied by deleting a tool. v2.4.0: `xor_key_length`, `cyclic_pattern`, `hash_identify`, `rsa_attack`. v3.3.0 adds twelve: `classical_cipher`, `corpus_diff`, `crib_drag`, `entropy_scan`, `hash_crack`, `hash_statistics`, `jwt_weakness`, `plaintext_check`, `rsa_multi_key`, `substitution_break`, `timestamp_identify`, `vigenere_break`. v3.4.0 adds `ecdsa_recover`; v3.8.0 adds `cert_chain`; v3.11.0 adds `pqc_identify`. Three kinds of gap: a **loop with a decision** inside it (the two cipher solvers), a statistic computed **across inputs** (`corpus_diff`, `rsa_multi_key`, `hash_statistics`, `ecdsa_recover`, `cert_chain`), and a primitive upstream simply lacks (`classical_cipher`, `pqc_identify` -- `src/core` has no FIPS 203/204/205 anything, and the ML-KEM/ML-DSA/SLH-DSA OID and size table came out of DER that Node 24 generated, not out of a specification read by eye) -- which is a registry tool rather than an operation because `src/core/**` is mirrored, so each would be a `patches/fork` patch that has to keep applying forever. `src/node/tools/lib/english.mjs` is the shared language model; its trigram table is **generated** by `scripts/build-english-trigrams.mjs` from this repository's own prose and must stay regenerable byte-for-byte (the generator once read its own base64 output back in as English -- base64 contains `//`). Loading is an explicit import list in `src/node/tools/index.mjs` (**not** `src/node/index.mjs`, which is the generated operation bridge) -- **no loader, no directory scan, no path from configuration.** `node:vm` is not a security boundary and this was measured: a capability passed into a vm context reaches the real `process` via `constructor`. Registration **throws** if a tool would shadow an operation or meta-tool. [ADR 0002](docs/adr/0002-tool-registry-is-not-a-plugin-loader.md). |
...
4a. **`cyberchef_analyse`** (v4.1.0) - Dispatches to any of the 19 registry tools by name, accepting the bare or `cyberchef_`-prefixed spelling. The ONLY route to them on the default `index` surface, which stopped listing them in v4.1.0; they are still listed outright on `curated`/`all`. It resolves the tool and then runs the **same statements** a direct call runs rather than a parallel copy, which is what makes "behaves identically" checkable -- verified byte-identical through a real client. Priced by the tool it selects, not its own annotations: it is in `RECIPE_SCOPED_TOOLS` beside `bake` and `batch`, because authorising a dispatcher as itself would check a wrapper while the work runs against the caller's choice. It belongs there and `recipe_execute` does not, mechanically: `analyse` carries a NAME resolvable against a Map fixed at construction, so the check stays first, while an id would move it after a storage read.
4b. **The dispatch table** (v4.2.0) - a meta-tool declaring a `run` in `META_TOOLS` is dispatched from that declaration, so name, description, schema and handler are one entry instead of a literal plus a branch nothing checked agreed with it. **Eight of the ten recipe tools**, not all ten: `recipe_execute` guards input size first and returns the recipe's own string rather than a JSON envelope, and `recipe_export` already returns a string that the table's `JSON.stringify` encoded a second time -- which round-trips through `recipe_import`, so folding it in broke a pair. Both keep their branches, with the reason written where the next person will try to remove them. `tests/mcp/dispatch-table.test.mjs` is the gate and it is **behavioural**: it calls every advertised tool through a real client and distinguishes "the handler rejected your arguments" from "no such tool". The syntactic version was tried first and flagged `cyberchef_analyse`, which has no top-level branch by design. It supersedes `meta-tool-parity.test.mjs` (v3.7.0), which asserted the same two directions -- so the v4.2.0 charter's "the dispatch table has never had its version" was false -- and which the refactor **breaks**, being a grep for `if (name === ...)`: a gate that forbids the change it protects.
5. **Removed in v4.0.0**: `cyberchef_deprecation_stats` and `cyberchef_migration_preview` (the v2.0.0 migration tools, added v1.8.0) and the `cyberchef-migrate` bin. They were advertised on every surface and cost 995 bytes of every `tools/list` for a migration nine minors old; the only warning still emitted was `DEP007`, a WITHDRAWN code reading "No action required". `src/node/deprecation.mjs` went with them. A negative test in `in-process-handlers.test.mjs` is the tripwire.
...
3. `deploy/helm/cyberchef-mcp/Chart.yaml` `appVersion` -- **and NOT the chart's own `version`**, which tracks the CHART and moves only when the templates change. This line said "and the chart's own `version`" until v3.9.0, contradicting both mechanisms that actually implement it: `bump-version.mjs` prints "NOT touched: the chart's own `version` (it tracks the chart, not the app)" and `check-version-consistency.mjs` reports it as "not checked, moves with the templates". A reviewer read this row and asked for a chart bump this release, which was the right question to ask of a row that was wrong
...
# ghcr.io/doublegate/cyberchef-mcp_v4, v2.x to ..._v2, the v1.9.x line to ..._v1.
...
| Guides | `docs/guides/commands.md` (MCP tools), `user_guide.md` (installation), `edge-deployment.md` (arm64, size, offline, air-gapped), `configuration.md` (all 61 settings; generated from `config-file.mjs` and asserted against it) |
| Planning | `docs/planning/v4/` (**current**: [`v4.x-plan.md`](docs/planning/v4/v4.x-plan.md), the measured plan for v4.1.0-v4.9.0, rewritten 2026-09-11 after v4.0.0 shipped. Its central finding reorders the line: **19 registry tools occupied 68% of the index** -- so the tool programme had a prerequisite nobody had costed, and it was v4.1.0, **shipped**. The plan's companion claim, that at 59 registry tools the index would cost more than `curated`, is **arithmetically impossible** and is corrected in place (F-02): registry tools are listed on every surface, so they grow all three together and the gap never moves. It also records that the documented reason registry tools are all listed is **half wrong** (`search` finds them; `describe_operation` refuses and its redirect assumes listing), and corrects `SEP-2419`, which this corpus had mislabelled as tool result contracts -- it is `cache_hint`, and **no result-shape SEP exists**. `v4.0.0-plan.md` stays, SUPERSEDED IN PART: it concluded v4.0.0 should not be cut, and v4.0.0 shipped anyway on **T-13**, an internal trigger it did not have. Nine charters, three groups -- Unblock (v4.1.0-v4.2.0, triggers fired), Build (v4.3.0-v4.5.0, behind v4.1.0), Watch (v4.6.0-v4.9.0, nothing moved). `to-dos/` carries the standing trigger watch including T-13 and the upstream watch; `archive/` holds the two charters whose content shipped as v3.10.0 and v3.11.0). `docs/planning/v3/` (**the ritual lives here**: the v3.0.0 plan, [`RE-MEASURE.md`](docs/planning/v3/RE-MEASURE.md) -- which **all nine** v4 charters open by mandating, so it is not optional reading for any of them -- and `task-level-scoring.md`, the design that concludes AGAINST building an eval harness), `docs/planning/ROADMAP.md`. `docs/planning/future-releases/` and `phases/` are **historical** -- every file carries a dated banner saying what replaced it. |
...
| Releases | `docs/releases/v4.2.0.md` (latest), then `v4.1.0.md`, `v4.0.0.md`, `v3.11.0.md`, `v3.10.0.md`, `v3.9.0.md`, `v3.8.0.md`, `v3.7.0.md`, `v3.6.0.md`, `v3.5.0.md`, `v3.4.0.md`, `v3.3.0.md`, `v3.2.0.md`, `v3.1.0.md`, `v3.0.0.md`, `v2.10.0.md` ... `v2.0.0.md`, `v1.9.0.md` ... `v1.0.0.md` |
| Internal | `docs/internal/tech-debt-analysis-v1.6.1.md` (project health: 8.9/10) |
| Measurement | `conformance/README.md` (the external oracle and why its baseline is a baseline); `benchmarks/README.md` (`measure:surfaces`, `measure:results`, `benchmark:check` and the variance study behind the tolerance, including why 20% did not hold); `docs/internal/measurements/v3.8.0-arm64-first-run.md` (the first arm64 numbers on real hardware, and why one run is not a gate), `docs/internal/measurements/v3.5.0-same-host-comparison.md` (why the calibration-task idea was tested and discarded, what replaced it, and in v3.6.0 the noise floor AND detection curve that made it a gate), `v3.4.0-runner-baseline.md` (the runner-captured baseline, its cross-instance study AND the CI runs that disproved the 20% tolerance it argued for), `v3.1.0-baseline.md` (the superseded developer-machine study). **Two refactor instruments added in v4.2.0**: `list-contract-snapshot.mjs` dumps a whole `tools/list` response -- descriptions, schemas, annotations, titles and the ORDER the 2026-07-28 SHOULD rests on -- so two builds can be diffed on more than names, and `meta-tool-answers.mjs` calls all 27 meta-tool entry points and normalises the ids and timestamps that would otherwise make every diff dirty. Both carry the same warning: **compare sizes before believing a clean diff**, because two failed captures are also identical, which is exactly what the first run of the first one produced. Everything is in BYTES: no tokenizer has ever been in this repo. |
