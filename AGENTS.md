...
| Tests | **1,691 MCP (72 files)** + 241 Node-API + 2,289 operations + 9 CI-executed examples. Re-counted in v3.8.0: this row said 1,640 across 68 files, which was already stale before the release began (1,682/71 on the v3.7.0 tag). Nothing gates it -- `check:versions` covers version strings and operation counts, and `tool-surface-figures` covers the surface tuple; the test count is a third figure with no check behind it. Re-run `npm run test:mcp` rather than trusting this row. |
