...
**CyberChef MCP Server** (v3.1.0) - Fork of GCHQ CyberChef wrapping the Node.js API into an MCP
server. Exposes 504 operations (encryption, encoding, compression, forensics) as AI assistant tools.

| Metric | Value |
|--------|-------|
| MCP Version | 3.1.0 (single source: `package.json` `version`, read by `src/node/lib/config.mjs`). `mcpVersion` was removed in v2.2.0 -- npm requires `version` to be the published version, so the upstream base moved to `cyberchefUpstreamVersion`. |
...
| Tests | 1,427 MCP (54 files) + 241 Node-API + 2,289 operations + 9 CI-executed examples |
| Coverage | 95.4% lines / 89.8% branches / 96.4% functions / 96.1% statements. Thresholds raised in v2.3.0 from 75/70/90/75 to **95/89/96/96** (branches raised again in v2.7.0), with `src/node/lib/**` held separately at 99/94/100/99 -- the old numbers were twenty points below actual, so the gate could not fail. `perFile` is off deliberately; see `vitest.config.mjs` for why. Gated on **pull requests** since v2.2.0. |
...
| Releases | `docs/releases/v3.1.0.md` (latest), then `v3.0.0.md`, `v2.10.0.md`, `v2.9.0.md`, `v2.8.1.md` ... `v2.0.0.md`, `v1.9.0.md` ... `v1.0.0.md` |
| Internal | `docs/internal/tech-debt-analysis-v1.6.1.md` (project health: 8.9/10) |
| Measurement | `conformance/README.md` (the external oracle and why its baseline is a baseline), `benchmarks/tool-surface.mjs` (`npm run measure:surfaces` -- wire bytes per tool surface, in BYTES: no tokenizer has ever been in this repo) |
