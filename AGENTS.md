...
| Planning | `docs/planning/v4/` (**current**: the v4.0.0 plan, which carries a **SUPERSEDED IN PART** banner -- it concluded v4.0.0 was not scheduled, and v4.0.0 shipped anyway because all four of its triggers were EXTERNAL specification events and the project had no internal path to a major. It was cut on **T-13, accumulated removals**, added to the watch in that release. Triggers A-D and the kill criterion are NOT repealed. Plus nine thin charters v4.1.0-v4.9.0, all minors -- three of which have shipped, as v3.9.0/v3.10.0/v3.11.0 -- and `to-dos/` with a standing trigger watch). `docs/planning/v3/` (**the ritual lives here**: the v3.0.0 plan, `RE-MEASURE.md`, `task-level-scoring.md` -- the design that concludes AGAINST building an eval harness, and one-page charters through v4.0.0), `docs/planning/ROADMAP.md`. `docs/planning/future-releases/` and `phases/` are **historical** -- every file carries a dated banner saying what replaced it. |
