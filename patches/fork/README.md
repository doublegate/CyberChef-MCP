...
| `08-crypto-api-vendored.patch` | `src/core/lib/Hash.mjs`, `SM2.mjs`, and 5 operations (`DeriveHKDFKey`, `FlaskSessionSign`, `FlaskSessionVerify`, `HMAC`, `SM3`) | `crypto-api@0.8.5` cannot be loaded as published: its `main` names an `index` file the tarball does not contain, and its ESM sources use extensionless relative imports Node rejects. Upstream patches `node_modules` from `postinstall`; npm 12 blocks dependency install scripts, so that never runs for anyone installing from the registry and the server dies at startup. The package is vendored at `src/vendor/crypto-api/` (MIT) and these seven imports are repointed at it, which removes the last install-script requirement and unblocks npm as a distribution channel. | **Yes** — until crypto-api publishes a release that ships its `main` entry and resolvable specifiers. |
| `09-image-ops-return-pooled-buffer.patch` | 17 jimp-backed operations (`AddTextToImage`, `BlurImage`, `ContainImage`, `ConvertImageFormat`, `CoverImage`, `CropImage`, `DitherImage`, `FlipImage`, `ImageBrightnessContrast`, `ImageFilter`, `ImageHueSaturationLightness`, `ImageOpacity`, `InvertImage`, `NormaliseImage`, `ResizeImage`, `RotateImage`, `SharpenImage`) | Each ended `run()` with `return imageBuffer.buffer`. A Node `Buffer` is a *view*: for a small allocation `.buffer` is the shared pool, not the image. Measured: a 129-byte PNG came back as a **65,599-byte ArrayBuffer at byteOffset 32**, so `present()` saw no magic bytes and threw "Invalid file type." -- from the presenter, after `run()` had succeeded -- and the surplus 65 KB was whatever else the process had recently allocated, which on a multi-caller server is other callers' data. Sliced to the view's own window. Upstream fixed exactly this in `GenerateImage.mjs`, citing the Node docs, and left the 17 siblings. | **Yes** -- until upstream applies its own `GenerateImage.mjs` fix to the rest. |
| `10-add-text-to-image-node-fonts.patch` | `src/core/operations/AddTextToImage.mjs` | The operation loaded its bitmap fonts through webpack-only `import()` of `.fnt`/`.png` under `src/web/static/fonts/bmfonts/`, then built an absolute URL from `self.docURL`. This fork removed `src/web/` in v1.7.1 and runs under plain Node, so **every call failed** with "Error preparing fonts. (Cannot find module ...RobotoBlack72White.fnt)" while the tool stayed advertised in `tools/list`. Restoring the assets alone is not enough -- Node cannot `import()` a `.fnt` and there is no document URL. The fonts are vendored at `src/vendor/bmfonts/` (Apache-2.0) and the loader now calls `loadFont(<path>)`, which jimp supports directly in Node. | **Yes** -- while this fork ships no web assets. |
| `11-utils-readfile-accepts-file.patch` | `src/core/Utils.mjs` | `Utils.readFile`'s JSDoc promises a `File` and its Node branch was `return Buffer.from(file).buffer`, which throws `ERR_INVALID_ARG_TYPE` on one -- `Buffer.from` takes a string, Buffer, ArrayBuffer or array-like, and the `src/node/File.mjs` shim is none of those. `Chef.bake` presents every dish, and `displayFilesAsHTML` calls this once per member, so **every `List<File>` operation threw before returning anything**: `Unzip`, `Untar` and `Extract Files`, dead through this fork's whole life. The shim keeps its bytes in `.data`, which is the shape the shim's own constructor already unwraps (`Buffer.from(d.data)`). `.buffer` is sliced to the view's own window for the reason patch 09 documents at length -- a small `Buffer` is a window onto a shared 8 KB pool, and returning the pool showed up here as `RangeError: offset is out of bounds`. A native `Blob` is handled too, so passing one is not a new bug. | **Yes** -- until upstream's `readFile` accepts the type its own signature documents. |
