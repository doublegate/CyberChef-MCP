...
### Phase 6: Evolution (v2.8.0 - v4.0.0) - shipped September 2026
...
gantt
    ...
    v3.5.0 Measuring, Not Estimating    :done, 2026-09-04, 1d
    v3.6.0 The Gate, Measured          :done, 2026-09-04, 1d
    v3.7.0 Four Of Eleven Files        :done, 2026-09-04, 1d
    v3.8.0 Blockers That Were Not      :done, 2026-09-04, 1d
    v3.9.0 Checks That Lied            :done, 2026-09-10, 1d
    v3.10.0 The Landmine               :done, 2026-09-11, 1d
    v3.11.0 Post-Quantum Identification :done, 2026-09-11, 1d
    v4.0.0 Retiring The Past            :done, 2026-09-11, 1d
```
...
| **v3.5.0** | Measuring, not estimating | **Released 2026-09-04.** Two of v3.4.0's conclusions were tested before being built on and both were wrong. The `server.json` gate it added was **a schema version behind on the day it shipped** (2025-09-29 against a current 2025-12-11), which the official `mcp-publisher validate` reports in one line -- so CI now runs the registry's own validator as an oracle beside the transcribed offline gate, the arrangement the conformance suite already had. And the benchmark fix it recorded, normalising against a calibration task, **would not have worked**: dividing out each run's median leaves a -44% residual against a raw -42%, because the hosts differ in profile rather than by a scale factor. Replaced by benchmarking the merge base and the head in one job on one runner -- tenfold tighter, and deliberately **not gating** until the same-host spread is measured on a runner. Also the **MCP registry listing**, which v3.4.0's ownership proofs made possible and nothing had used. | S | Low |
| **v3.6.0** | The gate, measured | **Released 2026-09-04.** The same-host benchmark comparison becomes a gate at **25% per task** -- a sentence written twice before here with a number that was wrong within days both times, so this release is mostly the measurement that makes it stick. Two halves: a **-7.6%** worst-case noise floor over four runs on unchanged code, and the detection curve no previous attempt ever measured -- a deliberate tunable slowdown in `To Hex` giving -8.2% at 10% extra work, -20.2% at 25%, -32.8% at 50%, with every untouched task inside the noise floor (which is what makes a PER-TASK threshold meaningful). Reach stated rather than implied: it fires above 25% slower, so **a quarter more work does not fail** and is printed instead. Also: a task with no usable measurement now fails rather than being dropped. First findings log in nine releases that opens "the plan was right". | S | Low |
| **v3.7.0** | Four of eleven files | **Released 2026-09-04.** v3.2.0 said it fixed the operation-count discrepancy; **nine live occurrences still said 505** two releases later, and `check:versions` reported every location `ok` throughout -- because it carried a hand-written **four-file list** and the claim appears in eleven. The check now DISCOVERS the files it checks (7 -> 37 locations), immediately finding two that reading by hand had missed, both operator-facing. The third phrasing anchor was chosen by measurement: a wide pattern flags twenty legitimate non-504 counts, while all nine wrong ones share the phrase "operation implementations". Also a guard for the meta-tool `META_TOOLS`/dispatch duplication the v3.0.0 plan recorded and six releases carried -- measured at 23 and 23, no drift, so a tripwire rather than a rewrite. **Nine carried-forward items were measured; two were already done and still being carried.** | S | Low |
| **v3.10.0** | The landmine under the test suite, and the number that gave it away | **Released 2026-09-11.** Executes the `v4.1.0` charter -- and ships as a v3 minor, because `4.1.0` cannot precede a `4.0.0` the plan beside it says not to cut. Every v4 charter now carries a banner saying the number is an ordering, not a schedule. `runServer()` was called at module scope with no main-module guard, so IMPORTING the server started one -- in all 23 test files that import it -- and armed `process.exit(1)` inside each. The guard resolves REALPATHS, because npm installs the `cyberchef-mcp` bin as a symlink and a naive compare would start nothing for every `npx` user while passing every test. The four temp directories v3.9.0 leaked as a workaround are reclaimed, verified by counting `/tmp`: 40 -> 40, where it was 40 -> 42. Fixing it **dropped coverage below the gate with nothing deleted** -- `runServer()` used to run on every test import, so about 0.6% of this project's line coverage was the bug executing itself, and the thresholds were calibrated partly against that. Recovered by testing the guard, not by moving the bar. Also: the registry publish now reads its sibling's state instead of trusting a 25-minute clock, and the v4 trigger watch is a script and a monthly job rather than twelve checks someone has to remember. | M | Low |
| **v3.11.0** | Post-quantum identification, and the margin that was warned about | **Released 2026-09-11.** Executes the `v4.2.0` charter (`ext-proj-int`), shipping as a v3 minor for the reason v3.10.0 recorded. `cyberchef_pqc_identify` is the nineteenth registry tool: it names the NIST parameter set behind a key, signature or ciphertext -- ML-KEM (FIPS 203), ML-DSA (FIPS 204), SLH-DSA (FIPS 205), all eighteen sets -- where **zero of the 504 operations touch any of them**, measured against `OperationConfig.json` rather than assumed. It identifies and does not implement: Node's `crypto` generates all eighteen sets from Node 24, so every fixture is generated at test time and the OID/size table was extracted from Node-produced DER, not transcribed. The first extraction scanned for a `0x06` tag byte and returned a plausible WRONG OID for ML-KEM-1024 -- five plausible results and one absurd one is a lucky failure mode -- replaced with a real DER walk. From raw bytes it reports every candidate and refuses to choose, because ML-KEM-1024's encapsulation key and ciphertext are both 1568 bytes and an SLH-DSA public key is the same length as a SHA-256 digest. **The thin coverage margin v3.10.0 documented lasted exactly one release**: statements fell to 94.99% against a threshold of 95, the deficit being the new parser's REJECTION paths (91.39% statements against 98.64% lines), and covering them honestly found a real defect -- the tool raised a bare `Error` where every sibling raises `createInputError`. | S | Low |
| **v4.0.0** | Retiring the past, and the trigger the plan never had | **Released 2026-09-11.** The first major since v3.0.0, and the trigger watch said none was due -- correctly, because all four of its triggers are EXTERNAL specification events, so the project had no internal path to a major at all. v4.0.0 waited on something that might never arrive while deprecated surface accumulated with no mechanism able to remove it. By v3.11.0 the only deprecation warning a normal `cyberchef_bake` call produced was **DEP007, a WITHDRAWN code reading "No action required"**, and the two migration tools it belonged to cost **995 bytes of every `tools/list`** -- two thirds of the entire new tool v3.11.0 had just shipped. Removed: both migration tools, the `cyberchef-migrate` bin (whose premise was false -- v1 positional, bare-string and v2 named recipe arguments all bake identically, verified through a real client), `src/node/deprecation.mjs` (522 lines), `CYBERCHEF_EXPOSE_ALL_OPS` (a silent alias that OUTRANKED `CYBERCHEF_TOOL_SURFACE`), the `compatibility` config section and `tools.exposeAllOps` (64 settings in 15 sections -> 61 in 14), and Node 24. **The round-trip multiplier improved for the first time since v3.2.0**, 8.9x -> 9.1x. Added **T-13**, the internal trigger, verified by planting the removed tool and watching it fire. Two gates earned their keep: `check:versions` caught five locations where the image had not been RENAMED to `_v4` (the v3.0.0 defect, first major since), and a T-13 test that mutated a source file raced `tool-surface-figures` in vitest's parallel runner -- passing alone and under coverage, failing the full suite in a file it never touched. | M | Medium |
| **v4.2.0** | One place to add a meta-tool, and two premises that were both wrong | **Released 2026-09-12.** Eight of the ten uniform recipe tools now dispatch from their `META_TOOLS` declaration -- one entry carrying name, description, schema and handler -- taking the dispatch chain from **24 `if` occurrences to 16**. Neither of the charter's two central claims survived contact. Its headline, *"the dispatch table has never had its version"*, was **false**: `meta-tool-parity.test.mjs` was built in v3.7.0 and already asserted both directions, so the release opened by duplicating a check -- and that gate is syntactic, so the refactor it was written to protect **breaks** it, a lock rather than a gate. It is superseded and removed, its one assertion behaviour cannot reach (a meta-tool shadowing a registry tool) carried forward and strengthened. Its second claim, that the ten branches *"differ only in the method called"*, was wrong for two: `recipe_execute` guards input size and returns a string, `recipe_export` was **double-encoded** by the table and round-trips through `recipe_import`, so folding them in dropped a guard and broke a pair. Caught by the pre-existing suite, **not** by the new gate -- the release's own kill criterion, *"a tidier dispatch that answers differently is a regression with good intentions"*, happening on the first attempt. Both keep their branches. Behaviour preservation was **proven, not asserted**: `tools/list` byte-identical on all three surfaces including order, `handleListTools` and its `visibleTools()` auth filter unchanged line for line, and 27 meta-tool calls through a real client byte-identical. `mcp-server.mjs` coverage 85.40% -> 85.75% statements and 68.04% -> 70.56% branches on **25 fewer statements and 10 fewer branches**. | S | Low |
| **v4.1.0** | Registry tools get a navigation path, and the charter's headline claim dies | **Released 2026-09-11.** The index falls **41 tools / 44,968 bytes -> 23 / 15,620**, 66%, with nothing made unreachable. Nineteen analysis tools were **68% of the default surface** and were there for a real reason: `describe_operation` REFUSED them and pointed at `tools/list`, so the listing was their only schema path and an unlisted tool could not be called at all. The dependency was removed before the tools -- `describe_operation` serves their schemas, `categories` lists them under `analysisTools`, and **`cyberchef_analyse`** dispatches by name, sharing ONE BODY with the direct call so "behaves identically" is checkable rather than aspirational (verified byte-identical). It is priced by the tool it selects, not its own annotations, or a scope-filtering bypass would have shipped in the same release that stopped listing the tools behind it. **The charter's headline claim was arithmetically impossible and its own gate found it**: forty synthetic registry tools grew `index`, `curated` and `all` by the same 76,440 bytes and the gap moved by ZERO, because registry tools are listed on every surface -- so the index cannot cross `curated` at any number, and the `1.01x` fix merged in #135 was polishing a table that should not exist (F-02). What survived needed no comparison: the 68% share, and a 41-tool index inside the **30-50 band where Anthropic measures tool-selection accuracy degrading**, which made the status quo the degraded condition. Round-trip multiplier **9.1x -> 24.5x**, the highest it has ever been. Also fixed a 100-session leak in the auth test helper, invisible until the first test large enough to cross the cap. | M | Low |
| **v3.9.0** | Checks that reported something other than what they said | **Released 2026-09-10.** No charter named this version, so `RE-MEASURE.md` step 6 applied and the scope came from measurement: the carried-forward list, the issue tracker, the Dependabot queue and a full dependency sweep. Three checks were green while reporting something false. `upstream-monitor.yml` compared a **release tag** against unreleased **master**, so issue #129 announced a v11.4.0 this repository was already on and cited a new operation that is not in the v11.4.0 tag -- and underneath that, it read `package.json` `version` (this fork's semver) where it meant `cyberchefUpstreamVersion`, so the comparison could never be equal and the `Already up-to-date` branch has never once executed. `cyberchef_magic` ranked a decoded plaintext **below the container it came out of** (#130), because upstream's comparator clamps any file-signature match to 500 while text keeps chi-squared scores in the thousands; recipe length looked like the cause and the arithmetic disproved it, and the first fix regressed a working case by promoting any longer UTF-8 result. And the surface-figures gate could only prove documents agreed with **each other** -- so v3.8.0 shipped an image serving 44,493 bytes while every document said 44,406, verified by A/B against the published artefact. All dependencies taken to their latest compatible versions, including vitest 5 seven days into a fourteen-day cooldown on measured evidence, with three majors held and each hold re-verified against upstream. | M | Low |
| **v3.8.0** | Two blockers that were not blockers | **Released 2026-09-04.** Two of five carried-forward items were **never actually blocked**. arm64 performance was recorded as needing "arm64 **hardware**" for five releases while `mcp-docker-build.yml` had been running jobs on `ubuntu-24.04-arm` the whole time -- three lines below a careful comment reasoning about that very runner label. It now has a benchmark job that **reports and does not gate**, because there is no arm64 noise floor yet and this project has set three thresholds from a first plausible measurement and moved two within days. Task-level scoring was blocked on a document nobody had written; it is written, and it **concludes against building the harness** -- the same-host construction that rescued the benchmark gate does not transfer, because host speed is a shared factor that cancels within a run while model sampling noise is independent per invocation. Also `cert_chain`, the 18th registry tool, whose design changed four times before shipping and never once because of review in the abstract. The serious one: the tool claimed `checkIssued()` verifies signatures and withdrew its "names match, signature fails" warning as unreachable on that basis. **It does not verify signatures** -- it compares names, key identifiers and key usage -- and the imposter that "proved" it had been rejected for a key-identifier mismatch mistaken for cryptography. Edge selection took the first `checkIssued` match, so a certificate with a forged subject AND matching `subjectKeyIdentifier` captured the link while the genuine issuer sat unconsidered in the same bundle: a good chain with one hostile certificate read as broken. Selection now requires a verifying signature; the warning is restored; the adversary is a committed fixture whose test fails against the old logic. A single negative result had been read as proof of a mechanism. | S | Low |
...
`v4.1.0` **shipped 2026-09-11** and `v4.2.0` **shipped 2026-09-12**, which closes **Phase 1** and the
Unblock group entirely. `v4.3.0`-`v4.5.0` were gated on v4.1.0 landing and are now unblocked;
`v4.3.0` (tools, slice 1) is the next charter, and it starts the way every one of them does, by
running [`RE-MEASURE.md`](./v3/RE-MEASURE.md) before any code. Nine charters in, it has found the
plan wrong nine times -- including twice in v4.2.0, whose two central claims both died.
