...
measurement and the conditions under which the decision should be revisited — a real isolate
boundary (a worker with no host objects passed in, or WASM), not a tidier vm.
...
One claim in that review did not survive checking. `hash_identify`'s hex fallback was flagged as
...
at 4 KB — because no hash is 8 MB, not because it is slow.
...
| Gate | Result |
|---|---|
| MCP tests | 1,094 across 39 files |
| Node API tests | 241 |
| Operation tests | 2,289 |
| Coverage | 95.62% statements / 88.68% branches / 96.38% functions / 96.48% lines |
| Lint | clean |
