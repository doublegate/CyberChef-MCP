...
over one input. That shape cannot express an **analysis**: scoring forty candidate key lengths,
...
|---|---|---|
| `cyberchef_xor_key_length` | `xortool` | Score every key length from 1 to 40 and rank them |
...
any of the 506 operation tools or any meta-tool, rather than resolving it by import order. Without
...
| Gate | Result |
|---|---|
| MCP tests | 1,087 across 39 files |
| Node API tests | 241 |
| Operation tests | 2,289 |
| Coverage | 95.64% statements / 88.57% branches / 96.61% functions / 96.51% lines |
| Lint | clean |
