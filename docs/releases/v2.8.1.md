...
operation are byte-for-byte what v2.8.0 shipped.
...
The published image runs **Node 26.8.1**. Every workflow tested **24**.
...
**Nothing to do.** No runtime code, dependency or image content changed from v2.8.0. If you are
pinning by digest, the digest changed only because the version label did.
