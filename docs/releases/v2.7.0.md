...
```text
npm run lint       clean
npm run test:mcp   1310 passed (50 files)
npm run test:coverage  95.78% stmts · 89.97% branches · 96.62% funcs · 96.63% lines — gate passes
                       src/node/lib/** 99.36 / 94.95 / 100.00 against a 99 / 94 / 100 floor
cold start         147-164 ms over 6 runs (v2.6.0 baseline: ~185 ms)
```

Seven properties verified by mutation rather than assumed: restoring the duplicate-HELP loop,
...
dashboard panels each fail the tests that cover them.
