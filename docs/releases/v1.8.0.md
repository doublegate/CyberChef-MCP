...
> `cyberchef_` prefix is permanent and the two meta-tools keep their names. Removing the prefix was
> measured at 2.6% of the `tools/list` payload (1,208 of 183,115 bytes) against 19 colliding tool
> names in MCP's flat namespace and a break for every existing integration. This page is left as
> the historical record of what v1.8.0 announced; see
> [docs/v2.0.0-breaking-changes.md](../v2.0.0-breaking-changes.md) for what actually shipped.
