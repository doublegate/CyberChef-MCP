...
The GHCR package is major-versioned: `cyberchef-mcp_v3` for v3.x, `cyberchef-mcp_v2` for the
v2.x line, `cyberchef-mcp_v1` for the frozen v1.9.x line. `mcp-release.yml` derives the suffix from the tag being built, so a `v1.9.x`
