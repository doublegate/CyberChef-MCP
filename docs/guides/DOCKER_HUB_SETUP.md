...
The GHCR package is major-versioned: `cyberchef-mcp_v4` for v4.x, `cyberchef-mcp_v3` for the
v3.x line, `cyberchef-mcp_v2` for v2.x, `cyberchef-mcp_v1` for the frozen v1.9.x line. `mcp-release.yml` derives the suffix from the tag being built, so a `v1.9.x`
