...
a saved recipe runs. That is the Node API loading all every operation implementation, deferred in
...
docker run -i --rm --network none \
  ...
  ghcr.io/doublegate/cyberchef-mcp_v4:4.0.0
```
...
```bash
docker pull --platform linux/arm64 ghcr.io/doublegate/cyberchef-mcp_v4:4.0.0
docker save ghcr.io/doublegate/cyberchef-mcp_v4:4.0.0 | gzip > cyberchef-mcp-arm64.tar.gz
```
...
docker buildx imagetools inspect ghcr.io/doublegate/cyberchef-mcp_v4:4.0.0

# That the running image is the architecture you expect
docker image inspect ghcr.io/doublegate/cyberchef-mcp_v4:4.0.0 --format '{{.Architecture}}'

# That it serves -- a LIVENESS probe, not a conformance check
echo '{"jsonrpc":"2.0","id":1,"method":"tools/list","params":{}}' \
  | docker run -i --rm ghcr.io/doublegate/cyberchef-mcp_v4:4.0.0
```
