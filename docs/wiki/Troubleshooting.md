...
## I only see about 24 tools, not 500

**Working as intended.** `tools/list` is an *index* by default: about 24 tools and ~3,400 tokens,
rather than 527 tools and ~98,000. Nothing is unreachable — `cyberchef_bake` runs any of the 504
...
Pre-load more if you want to: `CYBERCHEF_TOOL_SURFACE=curated` (~102 tools) or `=all` (all 527).
